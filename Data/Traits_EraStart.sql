-- Add default starting eras to timeline for missing players

INSERT OR REPLACE INTO HistoricalSpawnEras (Era, Civilization)
	SELECT '0', LeaderType
	FROM CivilizationLeaders WHERE EXISTS (SELECT * FROM Civilizations WHERE CivilizationLeaders.CivilizationType = Civilizations.CivilizationType AND StartingCivilizationLevelType = 'CIVILIZATION_LEVEL_FULL_CIV') AND NOT EXISTS (SELECT * FROM HistoricalSpawnEras WHERE Civilization = CivilizationType) AND NOT EXISTS (SELECT * FROM HistoricalSpawnEras WHERE Civilization = LeaderType);
...
INSERT OR REPLACE INTO Traits (TraitType, Name, Description)
	SELECT	'TRAIT_HSD' || '_' || LeaderType, 'LOC_HSD_ERASTART_NAME', (SELECT Name FROM Eras WHERE ChronologyIndex = (SELECT Era + 1 FROM HistoricalSpawnEras WHERE Civilization = LeaderType))
	FROM Leaders WHERE InheritFrom='LEADER_DEFAULT' AND EXISTS (SELECT * FROM HistoricalSpawnEras WHERE Civilization = LeaderType);
	
INSERT OR REPLACE INTO Traits (TraitType, Name, Description)
	SELECT	'TRAIT_HSD' || '_' || LeaderType, 'LOC_HSD_ERASTART_NAME', (SELECT Name FROM Eras WHERE ChronologyIndex = (SELECT Era + 1 FROM HistoricalSpawnEras WHERE Civilization = CivilizationType))
...
INSERT OR REPLACE INTO Traits (TraitType, Name, Description)
	SELECT	'TRAIT_HSD' || '_' || LeaderType, 'LOC_HSD_ERASTART_NAME', (SELECT Name FROM Eras WHERE ChronologyIndex = (SELECT Era + 1 FROM HistoricalSpawnEras WHERE Civilization = LeaderType))
	FROM Leaders WHERE InheritFrom IN ('LEADER_MINOR_CIV_CULTURAL', 'LEADER_MINOR_CIV_INDUSTRIAL', 'LEADER_MINOR_CIV_MILITARISTIC', 'LEADER_MINOR_CIV_RELIGIOUS', 'LEADER_MINOR_CIV_SCIENTIFIC', 'LEADER_MINOR_CIV_TRADE') AND EXISTS (SELECT * FROM HistoricalSpawnEras WHERE Civilization = LeaderType);
