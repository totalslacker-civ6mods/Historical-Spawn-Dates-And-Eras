...
-- INSERT OR REPLACE INTO Traits (TraitType, Name, Description)
	-- SELECT	'TRAIT_HSD' || '_' || LeaderType, 'LOC_HSD_NAME', (SELECT StartYear FROM HistoricalSpawnDates_LeaderHSD WHERE Civilization = LeaderType)
	-- FROM Leaders WHERE InheritFrom='LEADER_DEFAULT';
...
INSERT OR REPLACE INTO Traits (TraitType, Name, Description)
	SELECT	'TRAIT_HSD' || '_' || LeaderType, 'LOC_HSD_NAME', (SELECT StartYear FROM HistoricalSpawnDates_LeaderHSD WHERE Civilization = LeaderType)
	FROM Leaders WHERE InheritFrom='LEADER_DEFAULT' AND EXISTS (SELECT * FROM HistoricalSpawnDates_LeaderHSD WHERE Civilization = LeaderType);
	
INSERT OR REPLACE INTO Traits (TraitType, Name, Description)
	SELECT	'TRAIT_HSD' || '_' || LeaderType, 'LOC_HSD_NAME', (SELECT StartYear FROM HistoricalSpawnDates_LeaderHSD WHERE Civilization = CivilizationType)
...
INSERT OR REPLACE INTO Traits (TraitType, Name, Description)
	SELECT	'TRAIT_HSD' || '_' || LeaderType, 'LOC_HSD_NAME', (SELECT StartYear FROM HistoricalSpawnDates_LeaderHSD WHERE Civilization = LeaderType)
	FROM Leaders WHERE InheritFrom IN ('LEADER_MINOR_CIV_CULTURAL', 'LEADER_MINOR_CIV_INDUSTRIAL', 'LEADER_MINOR_CIV_MILITARISTIC', 'LEADER_MINOR_CIV_RELIGIOUS', 'LEADER_MINOR_CIV_SCIENTIFIC', 'LEADER_MINOR_CIV_TRADE') AND EXISTS (SELECT * FROM HistoricalSpawnDates_LeaderHSD WHERE Civilization = LeaderType);
